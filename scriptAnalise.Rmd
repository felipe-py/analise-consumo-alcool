---
title: "Análise de Consumo de Álcool entre estudantes da UEFS"
output:
  html_document:
    df_print: paged
  pdf_document:
...
Regras para que a exportação do script em .tex funcione corretamente.

```{'{r}
knitr::opts_chunk$set(
  echo = FALSE,      # (A OPÇÃO MAIS IMPORTANTE) NÃO mostra o seu código R no output.
  message = FALSE,   # NÃO mostra mensagens de status (ex: a mensagem do tidyverse ao carregar).
  warning = FALSE,   # NÃO mostra mensagens de aviso (ex: o aviso do teste Qui-Quadrado).
  fig.align = "center", # (Bônus) Centraliza todas as figuras e gráficos.
  fig.width = 8,       # (Bônus) Define uma largura padrão para as figuras.
  fig.height = 6       # (Bônus) Define uma altura padrão para as figuras.
)'''
```
...
-   Idade que começou a consumir
    ...
    ```{r}
    # Este código deve estar no seu bloco de preparação de dados
    ...
    # Aplica a ordenação ao data frame
    dados_brutos <- dados_brutos %>%
      mutate(IdadeQueComecou = factor(IdadeQueComecou, levels = niveis_idade_inicio))

    # --- Análise Univariada: Idade de Início do Consumo ---

    # 1. CÁLCULO (Seu código aqui está perfeito)
    ...
    # 2. TABELA (Seu código aqui está perfeito)
    knitr::kable(
      tabela_idade,
      caption = "Idade que começou a consumir"
    )

    # 3. GRÁFICO (Versão Corrigida)
    ggplot(tabela_idade, aes(x = `Idade que comecou`, y = Quantidade)) + # <-- MUDANÇA 1: reorder() removido
      geom_bar(stat = "identity", fill = "#5DADE2", alpha = 0.8) +
      geom_text(
        aes(label = paste0(Quantidade, "\n(", Percentual, ")")),
        vjust = 1.5,
      ...
      labs(
        title = "Idade de Início do Consumo de Bebida Alcoólica",
        x = "Idade de Início", # Rótulo do eixo X mais limpo
        y = "Número de Respondentes"
      ) +
      theme_minimal(base_size = 9) +
      
      # --- MUDANÇA 2: Linha adicionada para girar o texto do eixo X ---
...
-   Pegou carona com alguém sob efeito de álcool vs. Dirigiu sob efeito de álcool
    ...
    ```{r}
    dados_consumidores <- dados_brutos %>%
      filter(RespondenteIngere == "Sim")

    tabela_carona_vs_dirigir <- dados_consumidores %>%
      filter(DirigiuSobEfeitoDoAlcool %in% c("Sim", "Não") & PegouCaronaComAlguemSobEfeitoDaBebida %in% c("Sim", "Não")) %>%
      
      group_by(PegouCaronaComAlguemSobEfeitoDaBebida) %>%
      count(DirigiuSobEfeitoDoAlcool) %>%
      mutate(
        PercentualNumerico = n / sum(n),
        PercentualTexto = scales::percent(PercentualNumerico, accuracy = 0.1)
      ) %>%
    ...
    tabela_contingencia <- dados_consumidores %>%
      filter(DirigiuSobEfeitoDoAlcool %in% c("Sim", "Não") & PegouCaronaComAlguemSobEfeitoDaBebida %in% c("Sim", "Não")) %>%
      count(PegouCaronaComAlguemSobEfeitoDaBebida, DirigiuSobEfeitoDoAlcool) %>%
      tidyr::pivot_wider(names_from = DirigiuSobEfeitoDoAlcool, values_from = n, values_fill = 0)
    ...
    ggplot(tabela_carona_vs_dirigir, aes(x = PegouCaronaComAlguemSobEfeitoDaBebida, y = Quantidade, fill = DirigiuSobEfeitoDoAlcool)) +
      geom_bar(stat = "identity", position = "fill", alpha = 0.8) +
      geom_text(
        aes(label = PercentualTexto),
        position = position_fill(vjust = 0.5),
      ...
      scale_fill_manual(
        ...
        name = "Dirigiu Alcoolizado?"
      ) +
      labs(
        title = "Relação entre Aceitar Carona de Risco e o Hábito de Dirigir Alcoolizado",
        x = "Já Pegou Carona com Alguém Sob Efeito de Álcool?",
        y = "Proporção de Respostas"
      ) +
      theme_minimal(base_size = 14)

    knitr::kable(
      tabela_carona_vs_dirigir,
      caption = "Tabela de cruzamento entre o hábito de aceitar carona de risco e o de dirigir alcoolizado."
    )
    ```
...
-   Respondente ingere vs. Dirigiu sob efeito de álcool
    ...
    niveis_idade_inicio <- c("Menos de 12 anos", "12 a 14 anos", "15 a 17 anos", "18 a 20 anos", "21 a 25 anos", "Comecei a consumir álcool recentemente (nos últimos 6 meses)", "Outra")
    niveis_frequencia <- c("Fins de semana", "Três vezes ao mês", "Duas vezes ao mês", "Só em festas", "Outra", "Não consumo bebidas alcoólicas")

    dados_brutos <- dados_brutos %>%
      mutate(
        IdadeQueComecou = factor(IdadeQueComecou, levels = niveis_idade_inicio),
        Frequencia = factor(Frequencia, levels = niveis_frequencia)
      )

    dados_consumidores <- dados_consumidores %>%
      mutate(
        IdadeQueComecou = factor(IdadeQueComecou, levels = niveis_idade_inicio),
        Frequencia = factor(Frequencia, levels = niveis_frequencia)
      )
    ...
    ggplot(tabela_risco_dirigir, aes(x = Resposta, y = Quantidade, fill = Resposta)) +
      ...
      scale_fill_manual(values = c("Não" = "#3498DB", "Sim" = "#E74C3C")) +
      labs(
